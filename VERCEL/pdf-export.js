...
function generatePDF(){
  ...
  const sections=[
    { title:'Marché immobilier', color:'#b8832a', rows:[] },
    { title:'Population & économie', color:'#2980b9', rows:[] },
    { title:'Cadre de vie', color:'#27ae60', rows:[] },
    { title:'Mobilité & services', color:'#8e44ad', rows:[] },
  ...
  if(aq?.aqi) s2.push(['Qualité de l\'air','AQI '+aq.aqi+' - '+strip(aq.label||''),aq.aqi<=30?'G':aq.aqi<=60?'O':'R']);
  ...
  if(eco2?.total) s3.push(['Établissements scol.',eco2.total+' etablissements',eco2.total>=5?'G':eco2.total>=2?'O':'R']);
  ...
  if(mel?.pyramideAges){const pyr=mel.pyramideAges;const tot=Object.values(pyr).reduce((a,v)=>a+v,0);if(tot>0){const yj=Math.round(((pyr.Y_LT15||0)+(pyr.Y15T24||0))/tot*100);const se=Math.round(((pyr.Y65T79||0)+(pyr.Y_GE80||0))/tot*100);const ac=100-yj-se;s5.push(['Profil démographique','Jeunes '+yj+'% - Actifs '+ac+'% - Seniors '+se+'%',null]);}}
